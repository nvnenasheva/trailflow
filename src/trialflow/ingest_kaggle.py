...
def main():
    ...
    ap.add_argument("--input", required=True, help="Path to Kaggle CSV (KaggleV2-May-2016.csv)")
    ap.add_argument("--output", default="data/processed/visits.parquet", help="Output Parquet path")
    args = ap.parse_args()
    ...
    def bin_col(name: str) -> pd.Series:
        return df[name].fillna(0).astype(int) if name in df.columns else pd.Series(0, index=df.index, dtype=int)
    ...
    if "gender" in df.columns:
        df["gender"] = (
            df["gender"].astype(str).str.upper().str[0].map({"M": "M", "F": "F"}).fillna("U")
        )
    else:
        df["gender"] = "U"
    ...
    if "no_show" not in df.columns:
        raise ValueError("Column 'No-show' (normalized to 'no_show') not found in input CSV.")
    df["no_show"] = (df["no_show"].astype(str).str.strip().str.upper() == "YES").astype(int)

    # site_id from Neighbourhood (hashed)
    if "neighbourhood" in df.columns:
        df["site_id"] = df["neighbourhood"].astype(str).apply(lambda x: _hash_label(x, prefix="site"))
    else:
        df["site_id"] = "site0001"

    # patient_pseudo_id
    df["patient_pseudo_id"] = df["patientid"].astype(str) if "patientid" in df.columns else "p0"

    # visit_id
    df["visit_id"] = df["appointmentid"].astype(str) if "appointmentid" in df.columns else (df.index + 1).astype(str)

    # first visit flag (within dataset)
    df = df.sort_values(["patient_pseudo_id", "date", "visit_id"])
    df["is_first_visit"] = (~df.duplicated(subset=["patient_pseudo_id"], keep="first")).astype(int)
    ...
    # 4) Unified output schema
    out_cols = [
        "visit_id", "patient_pseudo_id", "date", "site_id", "visit_type",
        "weekday", "lead_time_days", "is_first_visit",
        "sms_received", "scholarship", "hipertension", "diabetes",
        "alcoholism", "handicap", "gender",
        "age", "no_show",
    ]
    # Ensure all present
    for c in out_cols:
    ...
    print(f"Saved: {path_out} (rows={len(out)}, no_show_rate={out['no_show'].mean():.3f})")
